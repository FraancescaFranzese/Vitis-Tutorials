...
for idx = 1:num_core
    ...
    sim_data = int16(str2double(string(strsplit(cell2mat(pruned_cells)))))';
    ...
    tv_img((core_pxl_cnt+1):(core_pxl_cnt+length(tv_data))) = int16(tv_data);
    aie_sim_img((core_pxl_cnt+1):(core_pxl_cnt+length(sim_data))) = int16(sim_data);
...
err = abs(int16(aie_sim_img) - tv_img);
max_err = max(err(:));
ms_err = mean(err(:).^2);
fprintf('\nComparison of simulation with int16 test vectors:\n');
