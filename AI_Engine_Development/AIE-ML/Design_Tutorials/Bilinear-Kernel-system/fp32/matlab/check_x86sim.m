...
config_txt = fileread('../aie_component/src/config.h');
...
pxl_rem = max_idx + 1;
core_pxl_cnt = 0;
slack = 1;    % floating point values won't match exactly

% compare the files for each core
for idx = 1:num_core
    % get the test vector data
    tv_file = sprintf('../aie_component/data/output_%d_ref.txt',idx);
    fid = fopen(tv_file,'r');
    tv_data = int32(fscanf(fid,'%d'));
    fclose(fid);

    % get simulator output data
    sim_file = sprintf('data.txt',idx);
