...
% input test vectors A (.h file)
for idx = 1:ncore
fname = sprintf('../sw/input_%d_A.h', idx);
fid = fopen(fname, 'w');
fprintf(fid, 'int32_t v_input_A[1024*1024*2] = {\n');

if fid > 0
    otv = reshape(input_tv_A(:, idx), 2, length(input_tv_A(:, idx))/2);
    for i = 1:size(otv, 2)
        if i < size(otv, 2) 
            fprintf(fid, '%d, %d,\n', otv(1, i), otv(2, i));
        else 
...
% input test vectors B (.h file)
for idx = 1:ncore
fname = sprintf('../sw/input_%d_B.h', idx);
fid = fopen(fname, 'w');
fprintf(fid, 'int32_t v_input_B[1024*1024*2] = {\n');

if fid > 0
    otv = reshape(input_tv_B(:, idx), 2, length(input_tv_B(:, idx))/2);
    for i = 1:size(otv, 2)
        if i < size(otv, 2) 
            fprintf(fid, '%d, %d,\n', otv(1, i), otv(2, i));
        else 
...
% input test vectors C (.h file)
for idx = 1:ncore
fname = sprintf('../sw/input_%d_C.h', idx);
fid = fopen(fname, 'w');
fprintf(fid, 'int32_t v_input_C[1024*1024*2] = {\n');

if fid > 0
    otv = reshape(input_tv_C(:, idx), 2, length(input_tv_C(:, idx))/2);
    for i = 1:size(otv, 2)
        if i < size(otv, 2) 
            fprintf(fid, '%d, %d,\n', otv(1, i), otv(2, i));
        else 
...
% output test vectors (.h)
for idx = 1:ncore
fname = sprintf('../sw/output_%d_ref.h', idx);
fid = fopen(fname, 'w');
fprintf(fid, 'int32_t output_ref[1024*1024] = {\n');
